...
class ParallelIPCHandler:
    ...
    async def handle_interview(self, command_id: str, agent_id: int, prompt: str, platform: str = None) -> bool:
        ...
        if platform in ("twitter", "reddit"):
            ...
            if "error" in result:
                ...
                print(f"  Entrevista fallida: agent_id={agent_id}, platform={platform}, error={result['error']}")
                return False
            else:
                self.send_response(command_id, "completed", result=result)
                print(f"  Entrevista completada: agent_id={agent_id}, platform={platform}")
                return True
        ...
        if success_count > 0:
            ...
            print(f"  Entrevista completada: agent_id={agent_id}, plataformas exitosas={success_count}/{len(platforms_to_interview)}")
            return True
        else:
    ...
    async def handle_batch_interview(self, command_id: str, interviews: List[Dict], platform: str = None) -> bool:
        ...
        if results:
            ...
            print(f"  Entrevista por lotes completada: {len(results)} agentes")
            return True
        else:
            self.send_response(command_id, "failed", error="Ninguna entrevista tuvo éxito")
            return False
    ...
    def _get_interview_result(self, agent_id: int, platform: str) -> Dict[str, Any]:
        ...
        try:
            ...
            print(f"  Fallo al leer los resultados de la entrevista: {e}")
    ...
    async def process_commands(self) -> bool:
        ...
        if command_type == CommandType.INTERVIEW:
            ...
            self.send_response(command_id, "completed", result={"message": "El entorno se cerrará"})
            return False
...
def fetch_new_actions_from_db(
    ...
    try:
        ...
        print(f"Fallo al leer acciones desde la base de datos: {e}")
...
def _enrich_action_context(
    ...
    try:
        ...
        # 补充上下文失败不影响主流程
        print(f"Fallo al enriquecer el contexto de la acción: {e}")
...
def create_model(config: Dict[str, Any], use_boost: bool = False):
    ...
    if not os.environ.get("OPENAI_API_KEY"):
        raise ValueError("Falta configuración de API Key, por favor establece LLM_API_KEY en el archivo .env del directorio raíz del proyecto")
...
async def run_twitter_simulation(
    ...
    if not os.path.exists(profile_path):
        log_info(f"Error: el archivo de Profile no existe: {profile_path}")
        return result
    ...
    if max_rounds is not None and max_rounds > 0:
        ...
        if total_rounds < original_rounds:
            log_info(f"Número de rondas truncado: {original_rounds} -> {total_rounds} (max_rounds={max_rounds})")
    ...
    for round_num in range(total_rounds):
        ...
        if _shutdown_event and _shutdown_event.is_set():
            if main_logger:
                main_logger.info(f"Señal de salida recibida, deteniendo la simulación en la ronda {round_num + 1}")
            break
...
async def main():
    parser = argparse.ArgumentParser(description='Simulación paralela OASIS para ambas plataformas')
    parser.add_argument(
        '--config',
        type=str,
        required=True,
        help='Ruta al archivo de configuración (simulation_config.json)'
    )
    parser.add_argument(
        '--twitter-only',
        action='store_true',
        help='Ejecutar solo la simulación de Twitter'
    )
    parser.add_argument(
        '--reddit-only',
        action='store_true',
        help='Ejecutar solo la simulación de Reddit'
    )
    parser.add_argument(
        '--max-rounds',
        type=int,
        default=None,
        help='Número máximo de rondas de simulación (opcional, para truncar simulaciones largas)'
    )
    parser.add_argument(
        '--no-wait',
        action='store_true',
    ...
    log_manager.info("=" * 60)
    log_manager.info("Simulación paralela OASIS para ambas plataformas")
    ...
    log_manager.info(f"  - Rondas totales configuradas: {config_total_rounds}")
    if args.max_rounds:
        log_manager.info(f"  - Límite máximo de rondas: {args.max_rounds}")
        if args.max_rounds < config_total_rounds:
            log_manager.info(f"  - Rondas a ejecutar realmente: {args.max_rounds} (truncadas)")
    log_manager.info(f"  - Número de Agents: {len(config.get('agent_configs', []))}")

    log_manager.info("Estructura de logs:")
    log_manager.info(f"  - Log principal: simulation.log")
    log_manager.info(f"  - Acciones de Twitter: twitter/actions.jsonl")
    log_manager.info(f"  - Acciones de Reddit: reddit/actions.jsonl")
    log_manager.info("=" * 60)
    ...
    if wait_for_commands:
        log_manager.info("")
        log_manager.info("=" * 60)
        log_manager.info("Entrando en modo de espera de comandos - el entorno sigue en ejecución")
